...
echo "<select id='change_score'>
        <option value='qualifications' selected>".get_string('scorm_quiz_score_qualifications','scormfull')."</option>
        <option value='result'>".get_string('Tests_school_achievement','scormfull')."</option>
</select>";
...
$table2->attributes['class'] = 'generaltable mod_index';
        ...
        if(! $scormfull->threshold)
        {
            ...
            error("The scorm with id $cm->instance corresponding to this scormfull $id is missing");
                ...
                $table->data[] = array (("<a href=\"$CFG->wwwroot/user/view.php?id=$key&amp;course=$course->id\">"."$value2</a><br />"),$setjudg,(($getvalue*100)."%"), $calculation_results_sum,!$scormfull->report_add ? "-" : ($calculation_results_sum>=$scormfull->report_value ? get_string('strpass','scormfull'): get_string('starnopass','scormfull')));
            ...
            error(get_string('Courses_have_been_changed','scormfull'));
                ...
                if($getvalue>=((int)$scormfull->thresholdvalue/100))
                        $setjudg= get_string('strpass','scormfull');
                         ...
                         $table->data[] = array (("<a href=\"$CFG->wwwroot/user/view.php?id=$key&amp;course=$course->id\">"."$value</a><br />"),$setjudg,(($getvalue*100)."%"), $calculation_results,!$scormfull->report_add ? "-" : ($calculation_results>=$scormfull->report_value ? get_string('strpass','scormfull'): get_string('starnopass','scormfull')));
                }
                $table2->data[] = array (("$connected_exam : ".$quiz->name));
                $table2->data[] = array (("$Thresholdvalue : ".$scormfull->thresholdvalue."%"));
        $report_data;
